...
const stylePatterns = [
  { regex: /\bvar\s+/, type: 'maintainability', severity: 'medium', title: 'Avoid var', message: 'var is function-scoped and can lead to subtle bugs; prefer let or const instead.' },
  { regex: /==\s*[^=]/, type: 'bug', severity: 'medium', title: 'Use strict equality', message: 'Loose equality can cause unexpected type coercion.' },
  { regex: /!=\s*[^=]/, type: 'bug', severity: 'medium', title: 'Use strict inequality', message: 'Loose inequality can cause unexpected type coercion.' }
];

export function analyzeStatically(code) {
  ...
  lines.forEach((line, index) => {
    ...
    for (const pattern of stylePatterns) {
      if (pattern.regex.test(line)) {
        findings.push(makeFinding(pattern.type, pattern.severity, lineNumber, pattern.title, pattern.message, 'Use a safer or more modern pattern.', line));
      }
    }
  ...
  return {
    summary: findings.length
      ? `Found ${findings.length} issues across security, maintainability, performance, and readability.`
      : 'No obvious static issues were found. AI review may still identify deeper concerns.',
    findings,
    scores
  };
}
